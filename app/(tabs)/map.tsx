/**
 * Map Screen — Tap-to-Navigate (Floating Card approach)
 *
 * WHY NOT CALLOUT BUTTONS?
 * ─────────────────────────────────────────────────────────────────────
 * react-native-maps <Callout> has a well-known Android bug: the Callout
 * wrapper intercepts ALL touch events before they reach child components
 * like TouchableOpacity. Buttons render but never fire onPress.
 *
 * THE FIX — render the action card OUTSIDE MapView:
 *   1. <Marker onPress> sets `selectedPOI` state
 *   2. <Callout> shows ONLY the name + distance (no buttons — just info)
 *   3. A floating card is rendered as a sibling of <MapView>, positioned
 *      at the bottom of the screen using absolute positioning
 *   4. Buttons in the floating card are normal React Native views —
 *      they work perfectly on both Android and iOS
 *   5. Tapping the map background (onPress on MapView) dismisses the card
 *
 * This is the standard pattern used by Google Maps, Apple Maps clones,
 * and every production react-native-maps app that needs working buttons.
 */
...
// ─── Floating POI Action Card ─────────────────────────────────────────────────
//
// Rendered as an absolute-positioned View that sits on TOP of the MapView
// as a sibling element. Because it is outside MapView, all touch events
// reach TouchableOpacity normally — no Callout interception.
...
function POIActionCard({ poi, onDismiss }: POICardProps) {
  ...
  return (
    <Animated.View
      style={[cardStyles.wrapper, { transform: [{ translateY: slideAnim }] }]}
    >
      {/* ── Dismiss handle ─────────────────────────────────────── */}
      ...
      {/* ── Header: type tag + distance + close ────────────────── */}
      <View style={cardStyles.header}>
        ...
        {poi.distanceText ? (
          <Text style={cardStyles.distance}>{poi.distanceText}</Text>
        ) : null}
      ...
      {poi.capabilities && poi.capabilities.length > 0 && (
        <View style={cardStyles.caps}>
          ...
          {poi.capabilities.length > 4 && (
            <Text style={cardStyles.capMore}>+{poi.capabilities.length - 4}</Text>
          )}
        </View>
      )}
      ...
      {/* ── Action buttons — OUTSIDE MapView so they always fire ── */}
      <View style={cardStyles.actions}>

        {/* Navigate — primary action, always shown */}
        <TouchableOpacity
        ...
        {/* Call — secondary, only when phone available */}
        {poi.phone ? (
          <TouchableOpacity
            style={[cardStyles.btn, cardStyles.btnCall]}
            onPress={() => callPOI(poi.phone!)}
            activeOpacity={0.8}
          >
      ...
      {/* Phone number shown as small text below buttons */}
      {poi.phone ? (
        <Text style={cardStyles.phoneHint}>{poi.phone}</Text>
      ) : null}
    </Animated.View>
  );
}
...
export default function MapScreen() {
  ...
  const [selectedPOI, setSelectedPOI]   = useState<POI | null>(null);  // ← NEW
  ...
  async function loadOnlineData(): Promise<void> {
    ...
    if (!cacheValid) {
      onlinePOIService
        .fetchAndCache(loc.lat, loc.lng, MAP_FETCH_RADIUS_M)
  ...
  async function refreshCachedPOIs(loc: StoredLocation): Promise<void> {
    const pois = await onlinePOIService.getCachedPOIs(
      loc.lat, loc.lng, 'all', MAP_FETCH_RADIUS_M / 1000
    );
  ...
  const displayPOIs: POI[] =
    isConnected && onlinePOIs.length > 0
      ? onlinePOIs.filter(p => p.type === filter)
      : offlinePOIs;

  const allFilters = isConnected
    ? [...CORE_FILTERS, ...ONLINE_ONLY_FILTERS]
    : CORE_FILTERS;

  const filterColor =
    allFilters.find(f => f.type === filter)?.color ?? Colors.brand.primary;

  const badge = getBadgeConfig(
    isConnected ? dataSource : 'offline',
    onlineLoading
  );
  ...
  return (
    <View style={styles.container}>
      ...
      {activeCrash && (
        <View style={styles.incidentBadge}>
          ...
          <Text style={styles.incidentBadgeText}>
            Live Incident ·{' '}
            {activeBystanderAlert?.distanceM
      ...
      <MapView
        ...
        // Tapping the map background dismisses the floating card
      ...
      >
        ...
        {displayPOIs.map(poi => {
          const pinColor =
            isConnected && onlinePOIs.length > 0
              ? (MARKER_COLOR[poi.type] ?? filterColor)
              : filterColor;
          ...
          return (
            <Marker
              ...
              pinColor={isSelected ? '#FFD700' : pinColor}   // gold tint when selected
              tracksViewChanges={false}
              // ── THE KEY CHANGE ────────────────────────────────────────
              // onPress sets selectedPOI → renders the floating card below.
              // NO <Callout> with buttons — that's what caused the bug.
              onPress={(e) => {
                e.stopPropagation();            // prevent MapView onPress firing
                setSelectedPOI(
            ...
            >
              {/*
               * <Callout> is kept but contains ONLY a text label — no buttons.
               * This shows the name bubble on tap, which is purely informational.
               * The actual Navigate/Call buttons are in the floating card below.
               *
               * On Android you can remove <Callout> entirely if you prefer
               * no bubble at all — the floating card handles everything.
               */}
              <Callout tooltip={false}>
                <View style={styles.simpleCallout}>
                  <Text style={styles.simpleCalloutText} numberOfLines={1}>
                    {poi.name}
                  </Text>
      ...
      {loaded && !selectedPOI && (
        <View style={styles.countBadge}>
          ...
          <Text style={styles.countText}>
            {displayPOIs.length} found · tap a pin for options
          </Text>
        </View>
      )}
      ...
      {userLoc && (
        <TouchableOpacity
          style={[
            styles.myLocBtn,
            // Shift up when card is visible so it doesn't overlap
            selectedPOI ? { bottom: Layout.CONTENT_BOTTOM_PADDING + 200 } : {},
          ]}
          onPress={() =>
            mapRef.current?.animateToRegion({
      ...
      {/* ── Floating POI action card ─────────────────────────────── */}
      {/* Rendered OUTSIDE MapView — buttons work on both platforms   */}
      {selectedPOI && (
        <POIActionCard
          poi={selectedPOI}
