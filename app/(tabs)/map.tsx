  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {activeCrash && (
          <>
            <Circle center={{ latitude: activeCrash.lat, longitude: activeCrash.lng }} radius={600} fillColor="rgba(255,59,48,0.06)" strokeColor="transparent" />
            <Circle center={{ latitude: activeCrash.lat, longitude: activeCrash.lng }} radius={300} fillColor="rgba(255,59,48,0.12)" strokeColor="rgba(255,59,48,0.30)" strokeWidth={1.5} />
            <Circle center={{ latitude: activeCrash.lat, longitude: activeCrash.lng }} radius={100} fillColor="rgba(255,59,48,0.25)" strokeColor="rgba(255,59,48,0.50)" strokeWidth={2} />
            <Marker coordinate={{ latitude: activeCrash.lat, longitude: activeCrash.lng }} anchor={{ x: 0.5, y: 0.5 }}>
              <View style={styles.incidentPin}><View style={styles.incidentPinCore} /></View>
            </Marker>
          </>
        )}
      </MapView>
...
const cardStyles = StyleSheet.create({
  wrapper: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingBottom: Layout.CONTENT_BOTTOM_PADDING - 20, ...Shadows.lg, zIndex: 20 },
  handleArea: { alignItems: 'center', paddingVertical: 10 },
  handle: { width: 36, height: 4, borderRadius: 2, backgroundColor: 'rgba(60,60,67,0.18)' },
  header: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  typeTag: { paddingHorizontal: 9, paddingVertical: 3, borderRadius: 6 },
  typeText: { fontSize: 10, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.4 },
  distance: { fontSize: 13, fontWeight: '600', color: Colors.label.secondary, flex: 1 },
  name: { fontSize: 20, fontWeight: '700', color: Colors.label.primary, letterSpacing: -0.4, marginBottom: 6, lineHeight: 26 },
  hoursRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 8 },
  hours: { fontSize: 12, color: Colors.label.tertiary },
  caps: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 8 },
  capBadge: { backgroundColor: Colors.background.grouped, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  capText: { fontSize: 10, color: Colors.label.secondary, textTransform: 'capitalize', fontWeight: '500' },
  divider: { height: 0.5, backgroundColor: 'rgba(60,60,67,0.12)', marginVertical: 14 },
  ...
  btnNavigate: { backgroundColor: Colors.brand.accent, shadowColor: Colors.brand.accent, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.30, shadowRadius: 10, elevation: 6 },
  btnNavigateText: { fontSize: 16, fontWeight: '700', color: '#FFFFFF' },
  btnCall: { backgroundColor: `${Colors.status.success}12`, borderWidth: 1.5, borderColor: `${Colors.status.success}40` },
  btnCallText: { fontSize: 16, fontWeight: '700', color: Colors.status.success },
  phoneHint: { fontSize: 11, color: Colors.label.tertiary, textAlign: 'center' },
});
...
const styles = StyleSheet.create({
  ...
  incidentBadge: { position: 'absolute', top: Layout.STATUS_BAR_HEIGHT + 52, alignSelf: 'center', zIndex: 10, flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: BorderRadius.full, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: `${Colors.brand.primary}30`, ...Shadows.sm },
  ...
  // ── PHASE 9 styles ──────────────────────────────────────────────────────────
  blackspotBadge: { position: 'absolute', top: Layout.STATUS_BAR_HEIGHT + 86, alignSelf: 'center', zIndex: 10, flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: BorderRadius.full, paddingHorizontal: 12, paddingVertical: 5, borderWidth: 1, borderColor: 'rgba(204,0,0,0.25)', ...Shadows.sm },
  blackspotBadgeText: { fontSize: 11, fontWeight: '600', color: '#CC0000' },
  // ─────────────────────────────────────────────────────────────────────────────
  incidentPin: { width: 20, height: 20, borderRadius: 10, backgroundColor: 'rgba(255,59,48,0.25)', alignItems: 'center', justifyContent: 'center' },
  incidentPinCore: { width: 10, height: 10, borderRadius: 5, backgroundColor: Colors.brand.primary, borderWidth: 2, borderColor: '#FFFFFF' },
  simpleCallout: { backgroundColor: '#FFFFFF', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, maxWidth: 200, ...Shadows.sm },
  simpleCalloutText: { fontSize: 13, fontWeight: '600', color: Colors.label.primary },
  simpleCalloutHint: { fontSize: 10, color: Colors.label.tertiary, marginTop: 2 },
  legend: { position: 'absolute', bottom: Layout.CONTENT_BOTTOM_PADDING + 70, left: Layout.HORIZONTAL_PADDING, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: BorderRadius.lg, padding: 10, gap: 5, ...Shadows.sm, borderWidth: 1, borderColor: Colors.border.subtle },
  legendRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  legendDot: { width: 8, height: 8, borderRadius: 4 },
  legendText: { fontSize: 11, color: Colors.label.secondary, fontWeight: '500' },
  countBadge: { position: 'absolute', bottom: Layout.CONTENT_BOTTOM_PADDING + 16, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: BorderRadius.full, paddingHorizontal: 16, paddingVertical: 8, ...Shadows.sm, borderWidth: 1, borderColor: Colors.border.subtle },
  countText: { fontSize: 13, color: Colors.label.primary, fontWeight: '500' },
  myLocBtn: { position: 'absolute', bottom: Layout.CONTENT_BOTTOM_PADDING + 60, right: Layout.HORIZONTAL_PADDING, width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.95)', alignItems: 'center', justifyContent: 'center', ...Shadows.md, zIndex: 10 },
});