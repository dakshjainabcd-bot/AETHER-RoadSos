 ...
 *
 * Feature launcher panel ("+" FAB) contains only:
 *   - AI First-Aid Chat (Pocket RAG)
 *   - Multilingual Bridge (Whisper STT)
 *   - Black Box (evidence & road repair)
 *
 * Removed from panel (have dedicated nav buttons already):
 *   BystAI+PsychAid, Driver Intelligence, DTN Mesh,
 *   Security & Privacy, Rakshak+EMS
 *
 * Emergency contacts badge restored.
 */
...
const SHOWCASE_SERVICES = [
  {
    id: 'sh1', name: 'AIIMS — Trauma Centre', type: 'hospital',
    dist: '2.1 km', phone: '011-26588500',
    tags: ['Trauma', 'Neuro ICU', '24×7', 'Emergency'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '42 beds free',
  },
  {
    id: 'sh2', name: 'City Police HQ — Control Room', type: 'police',
    dist: '3.4 km', phone: '100',
    tags: ['24×7', 'PCR Van', 'FIR', 'Women Cell'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.service.police,
    dotBg: Colors.soft.blue, dotBorder: Colors.soft.blueBorder, beds: null,
  },
  {
    id: 'sh3', name: 'Fortis Memorial Hospital', type: 'hospital',
    dist: '4.7 km', phone: '1800-103-4444',
    tags: ['Cardiac', 'Burns Unit', 'ICU', 'Paeds'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '18 beds free',
  },
  {
    id: 'sh4', name: 'Apollo Hospitals', type: 'hospital',
    dist: '5.2 km', phone: '1860-500-1066',
    tags: ['Spine', 'Neuro', 'Cardiac', 'Robotic Surgery'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '6 beds free',
  },
  {
    id: 'sh5', name: 'Fire Station — Sector 6', type: 'fire',
    dist: '1.8 km', phone: '101',
    tags: ['Rescue', 'Chemical', 'High-Rise', 'Ambulance'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.service.fire,
    dotBg: Colors.soft.amber, dotBorder: Colors.soft.amberBorder, beds: null,
  },
  {
    id: 'sh6', name: 'Max Super Speciality Hospital', type: 'hospital',
    dist: '6.1 km', phone: '011-26515050',
    tags: ['Multi-Organ', 'Oncology', 'NICU', 'Blood Bank'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '31 beds free',
  },
  {
    id: 'sh7', name: 'Medanta — The Medicity', type: 'hospital',
    dist: '8.9 km', phone: '0124-4141414',
    tags: ['Level 1 Trauma', 'Heart', 'Liver Transplant'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '12 beds free',
  },
  {
    id: 'sh8', name: 'Narayana Health City', type: 'hospital',
    dist: '11.4 km', phone: '1800-843-6600',
    tags: ['Paediatric Cardiac', 'Burns', 'Trauma ICU'], status: 'OPEN 24H',
    statusColor: Colors.status.success, dotColor: Colors.brand.primary,
    dotBg: Colors.soft.red, dotBorder: Colors.soft.redBorder, beds: '24 beds free',
  },
];

// ── Feature Launcher Items — only 3 (as intended) ────────────────────────────
...
export default function HomeScreen() {
  ...
  const [contactCount, setContactCount]        = useState(0);  // ← RESTORED
  ...
  // RESTORED: load emergency contacts count
  useFocusEffect(
    useCallback(() => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background.primary }}>
      ...
      >
        ...
        {contactCount > 0 ? (
          <TouchableOpacity
            style={styles.contactsBadge}
            onPress={() => router.push('/(tabs)/contacts' as any)}
          >
            <Ionicons name="people" size={12} color={Colors.status.success} />
            <Text style={styles.contactsBadgeTextStyle}>
              {contactCount} emergency contact{contactCount > 1 ? 's' : ''} set up
            </Text>
            <Ionicons name="chevron-forward" size={12} color={Colors.label.tertiary} />
          </TouchableOpacity>
        ) : (
          <TouchableOpacity
            style={styles.setupContactsBadge}
            onPress={() => router.push('/(tabs)/contacts' as any)}
          >
            <Ionicons name="people-outline" size={12} color={Colors.status.warning} />
            <Text style={styles.setupContactsBadgeTextStyle}>
              Add emergency contacts — notify family on SOS
            </Text>
            <Ionicons name="chevron-forward" size={12} color={Colors.label.tertiary} />
          </TouchableOpacity>
        )}
        ...
        {(nearestHospital || nearestPolice) && (
          <>
            ...
            {nearestHospital && (
              <ServiceRow poi={nearestHospital} dotColor={Colors.brand.primary} dotBg={Colors.soft.red} dotBorder={Colors.soft.redBorder} />
            )}
            {nearestHospital && nearestPolice && <View style={styles.rowDivider} />}
            {nearestPolice && (
              <ServiceRow poi={nearestPolice} dotColor={Colors.service.police} dotBg={Colors.soft.blue} dotBorder={Colors.soft.blueBorder} />
            )}
          </>
        )}
      ...
      <View style={styles.fabContainer} pointerEvents="box-none">
        <TouchableOpacity style={styles.fab} onPress={togglePanel} activeOpacity={0.85}>
      ...
      {/* ── Feature Launcher Panel (unchanged: Animated.View) ── */}
      {panelVisible && (
        <Modal transparent animationType="none" visible={panelVisible} onRequestClose={closePanel} statusBarTranslucent>
          <Animated.View style={[panelStyles.backdrop, { opacity: backdropOp }]} pointerEvents="auto">
            <Pressable style={{ flex: 1 }} onPress={closePanel} />
          </Animated.View>

          <Animated.View style={[panelStyles.sheet, { transform: [{ translateY: panelSlide }] }]}>
            <View style={panelStyles.handle} />

            <View style={panelStyles.panelHeader}>
              <View>
                <Text style={panelStyles.panelTitle}>More Features</Text>
                <Text style={panelStyles.panelSub}>AI tools · language bridge · evidence</Text>
              </View>
            ...
            <View style={panelStyles.grid}>
              {FEATURE_ITEMS.map((item) => (
                <TouchableOpacity
                  key={item.id}
                  style={[panelStyles.featureCard, { backgroundColor: item.bg, borderColor: item.border }]}
                ...
                >
                  ...
                  <Text style={panelStyles.featureSub} numberOfLines={2}>{item.sublabel}</Text>
                  {item.badge && (
                    <View style={[panelStyles.phaseBadge, { backgroundColor: `${item.color}18` }]}>
                      <Text style={[panelStyles.phaseBadgeText, { color: item.color }]}>{item.badge}</Text>
                    </View>
                  )}
                </TouchableOpacity>
              ))}
...
const panelStyles = StyleSheet.create({
  ...
  panelTitle: { fontSize: 22, fontWeight: '800', color: Colors.label.primary, letterSpacing: -0.5 },
  panelSub: { fontSize: 11, color: Colors.label.tertiary, marginTop: 2, letterSpacing: 0.3 },
  closeBtn: {
  ...
  featureCard: { width: '47%', borderRadius: 20, borderWidth: 1.5, padding: 14, gap: 5 },
  iconWrap: { width: 44, height: 44, borderRadius: 13, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  featureLabel: { fontSize: 13, fontWeight: '700', letterSpacing: -0.2, lineHeight: 17 },
  featureSub: { fontSize: 10, color: Colors.label.tertiary, lineHeight: 14 },
  phaseBadge: { alignSelf: 'flex-start', borderRadius: 5, paddingHorizontal: 6, paddingVertical: 2, marginTop: 4 },
  phaseBadgeText: { fontSize: 9, fontWeight: '700', letterSpacing: 0.5 },
});

// ── Main Styles (contacts badge styles restored) ─────────────────────────────

const styles = StyleSheet.create({
  scroll: { flex: 1, backgroundColor: Colors.background.primary },
  content: { paddingTop: Layout.STATUS_BAR_HEIGHT, paddingHorizontal: Layout.HORIZONTAL_PADDING, paddingBottom: Layout.CONTENT_BOTTOM_PADDING },

  fabContainer: { position: 'absolute', bottom: TAB_BAR_HEIGHT + 14, right: 20, zIndex: 99 },
  fab: {
  ...
  heroCard: {
    ...
    shadowColor: '#C82F1C', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 1, shadowRadius: 0, elevation: 4,
  },
  heroRing1: { position: 'absolute', right: -48, top: '50%', marginTop: -110, width: 220, height: 220, borderRadius: 110, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  heroRing2: { position: 'absolute', right: 0, top: '50%', marginTop: -70, width: 140, height: 140, borderRadius: 70, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  ...
  crashPill: { flexDirection: 'row', alignItems: 'center', gap: 7, alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 8, borderWidth: 1, marginBottom: 20 },
  crashDot: { width: 6, height: 6, borderRadius: 3 },
  crashText: { fontSize: 10, fontWeight: '700', letterSpacing: 1.5 },

  // RESTORED contacts badge styles
  contactsBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 7,
    backgroundColor: Colors.soft.green, borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 7,
    borderWidth: 1, borderColor: Colors.soft.greenBorder,
    marginBottom: 16,
  },
  contactsBadgeTextStyle: { fontSize: 12, color: Colors.label.secondary, flex: 1 },
  setupContactsBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 7,
    backgroundColor: Colors.soft.amber, borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 7,
    borderWidth: 1, borderColor: Colors.soft.amberBorder,
    marginBottom: 16,
  },
  setupContactsBadgeTextStyle: { fontSize: 12, color: Colors.label.secondary, flex: 1 },
  ...
  serviceAccent: { width: 42, height: 42, borderRadius: 14, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  ...
  serviceName: { fontSize: 14, fontWeight: '700', color: Colors.label.primary, lineHeight: 18, flex: 1, paddingRight: 8, letterSpacing: -0.2 },
  ...
  statusPill: { flexDirection: 'row', alignItems: 'center', gap: 4, borderRadius: 6, borderWidth: 1, paddingHorizontal: 7, paddingVertical: 2 },
  ...
  tag: { backgroundColor: Colors.background.secondary, borderWidth: 1, borderColor: Colors.border.medium, borderRadius: 6, paddingHorizontal: 9, paddingVertical: 3 },
  tagText: { fontSize: 9, fontWeight: '600', color: Colors.label.secondary, letterSpacing: 0.5 },
  serviceButtons: { flexDirection: 'row', gap: 8 },
  callBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 9, backgroundColor: Colors.soft.green, borderWidth: 1, borderColor: Colors.soft.greenBorder, borderRadius: 10 },
  callBtnText: { fontSize: 12, fontWeight: '600', color: Colors.status.success },
  navBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 9, backgroundColor: Colors.soft.blue, borderWidth: 1, borderColor: Colors.soft.blueBorder, borderRadius: 10 },
  ...
  locationWarn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: Colors.soft.amber, borderRadius: BorderRadius.md, padding: 12, marginBottom: 10 },
